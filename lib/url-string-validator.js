...
function isValidSchemeRelativeFileURLString(input) {
  ...
  if (afterSlashes.startsWith("/")) {
    return isValidPathAbsoluteURLString(afterSlashes);
  }

  const { authority, path } = splitOffPath(afterSlashes);
...
function isValidPathAbsoluteURLString(input) {
  return input.startsWith("/") && isValidPathRelativeURLString(input.substring(1));
}
